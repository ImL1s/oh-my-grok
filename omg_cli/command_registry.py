...
Phase 4′: every family registers parsers via ``register_*_parsers`` /
``register_note_parser``. ``main.build_parser`` only composes family hooks
(preserving help order with early/late phases where needed).
"""
...
COMMAND_SPECS: Final[tuple[CommandSpec, ...]] = (
    ...
    CommandSpec("session", "host-session argv plus search/replay (#74)", "run"),
    CommandSpec("trace", "read-only lifecycle event timeline (#74)", "run"),
    ...
    CommandSpec(
        "visual",
        "visual compare/capture/verdict/ralph/overlay (scored/blocked; #75)",
        "inspect",
    ),
    CommandSpec(
        "job",
