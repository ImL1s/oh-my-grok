...
def test_operation_catalog_v4_golden_unchanged() -> None:
    expected = json.loads(GOLDEN_V4.read_text(encoding="utf-8"))
    actual = serialize_operation_catalog(
        operations=TEAM_OPERATION_CATALOG_V4, schema_version=4
    )
    assert actual == expected
    assert (
...
def test_operation_catalog_matches_golden() -> None:
    ...
    assert any(op["name"] == "enqueue-host-prompt" for op in actual["operations"])
...
def test_operation_catalog_handler_coverage() -> None:
    ...
    assert "broadcast" in implemented
