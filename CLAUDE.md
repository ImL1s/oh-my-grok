...
- **Version bump**: edit `plugin.json`, regen `omg_capabilities.lock.json` (`generate_capabilities_lock.py`), add a CHANGELOG section, and re-run `omg setup` to refresh the installed `~/.grok/rules/omg.md` version. The **installed plugin snapshot** does NOT refresh via `grok plugin update` for a local-path install (it's a no-op) — reliable refresh is `grok plugin uninstall oh-my-grok --confirm && ./scripts/install-plugin.sh`. `omg doctor`'s version-drift + installed-capabilities-lock checks catch the staleness.
...
Per `docs/security-model.md` and the parity matrix: `capability_mode` is the primary isolation; PreToolUse is a fail-open soft-gate (not a sandbox). There is no OMC-style Stop hard-pin (Grok's Stop is passive), no full LSP/AST MCP bridge (`omg lsp` is a local probe). An **experimental grok-only** tmux team plane (`omg team`, behind `OMG_EXPERIMENTAL_TMUX_TEAM=1`) exists; panes run grok only; isolation is integration isolation (worktree ownership + seal + integrate), **not** an execution sandbox; multi-CLI executor panes are not yet present. `omg --madmax` is a break-glass host launcher, not a mode FSM.
