...
**Grok host contract shapes everything.** `PreToolUse` and `Stop` (grok **≥0.2.107**, autopilot incomplete, cap **8**/turn, fail-open) can return blocking decisions; every other Grok hook event is passive (its stdout is IGNORED — no context injection like Claude Code). Consequences that pervade the design:
...
- Continuity is a rules-file/CLI concern: `hooks/bin/session_start.py` is passive (stdout ignored; may spool events and **passively refresh** `.omg/state/RESUME.md` when an active non-terminal autopilot run exists — fail-open, never verifies); the `omg` CLI remains the authoritative RESUME writer; the model is told to read RESUME.md (nothing re-injects it).
...
- **Hooks must fail open**: `hooks/bin/*.py` never crash a session and never set `verified`. `stop.py` may emit `decision:block` via `omg_cli.stop_gate.decide_stop` while autopilot is incomplete (fail-open; honored **≥0.2.107**; cap **8**/turn). Kill switches: `DISABLE_OMG=1` (all hooks off), `OMG_SKIP_HOOKS="name,…"` (per-hook).
...
Per `docs/security-model.md` and the parity matrix: `capability_mode` is the primary isolation; PreToolUse is a fail-open soft-gate (not a sandbox). Stop pin on grok **≥0.2.107** pins the turn while autopilot is incomplete (cap **8**/turn, fail-open; not infinite OMC-style stickiness). No full LSP/AST MCP bridge (`omg lsp` is a local probe). A **default-on** tmux team plane (`omg team`; kill switch `OMG_DISABLE_TMUX_TEAM=1`, legacy `OMG_EXPERIMENTAL_TMUX_TEAM=0` disables) exists: zero-config is grok-only; `--routing` enables multi-CLI executor panes (codex/agy/cursor/gemini) with role floors (reviewer/verifier cannot use cursor; posture from role). Isolation is **integration** isolation (worktree ownership + seal + integrate), **not** an execution sandbox — provider CLI read-only strength is uneven (gemini: none; agy: best-effort). Live promotion proof: `LIVE_TEAM_SMOKE_OK` (local, not CI). `omg --madmax` is a break-glass host launcher, not a mode FSM.
