...
Per `docs/security-model.md` and the parity matrix: `capability_mode` is the primary isolation; PreToolUse is a fail-open soft-gate (not a sandbox). There is no OMC-style Stop hard-pin (Grok's Stop is passive), no full LSP/AST MCP bridge (`omg lsp` is a local probe), and no tmux multi-CLI team plane. `omg --madmax` is a break-glass host launcher, not a mode FSM.
