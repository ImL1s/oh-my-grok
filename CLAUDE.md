...
- Continuity is a rules-file/CLI concern: `hooks/bin/session_start.py` is passive/observe-only (may spool events; stdout is ignored); the `omg` CLI writes `.omg/state/RESUME.md`; the model is told to read it (nothing re-injects it).
