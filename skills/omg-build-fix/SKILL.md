---
name: omg-build-fix
description: Root-cause a broken build and apply a minimal fix. Use when the user says build-fix, fix the build, or broken build.
---

# omg-build-fix — Build-fix debugger slice

Root-cause a broken build and apply a minimal fix. Use when the user says build-fix, fix the build, or broken build.
...
## 1. Activation
- Phrases: `build-fix`, `fix the build`, `broken build`
- Aliases: `build-fix`
- **When:** Compile/test harness is red and the ask is to restore green for this slice.
- **Do not use when:** Feature work, or claiming verified because the build passed once.
- Informational questions (`what is omg-build-fix?`) do **not** activate this skill.
...
## 3. Runtime owner
- This playbook + omg-cli QA stamps. Never write passes/verified yourself.
...
- Failing command + log excerpt. Artifacts: `.omg/artifacts/build-fix.md`.
