---
name: omg-tdd
description: Red-green-refactor one failing test slice. Use when the user says tdd, test driven, or red green refactor.
---

# omg-tdd — TDD implementer slice

Red-green-refactor one failing test slice. Use when the user says tdd, test driven, or red green refactor.
...
## 1. Activation
- Phrases: `tdd`, `test driven`, `red green refactor`
- Aliases: `tdd`
- **When:** A failing test or new test is the acceptance needle for this slice.
- **Do not use when:** Full autopilot lifecycle, or marking the whole goal verified.
- Informational questions (`what is omg-tdd?`) do **not** activate this skill.
...
## 3. Runtime owner
- Implementers spawn `read-write` (no execute/all). QA evidence is CLI-owned.
...
- Failing test identity + slice scope. Artifacts: `.omg/artifacts/tdd-notes.md`.
