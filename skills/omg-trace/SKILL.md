---
name: omg-trace
description: Read OMG tracker/lifecycle projection. Use when the user says trace, trace the run, or wants generation-fenced status.
---

# omg-trace — Trace / lifecycle projection

Read OMG tracker/lifecycle projection. Use when the user says trace, trace the run, or wants generation-fenced status.
...
## 1. Activation
- Phrases: `omg trace`, `trace workflow`, `trace the run`
- Aliases: `trace`
- **When:** Need a passive view of run/project lifecycle without mutating verified.
- **Do not use when:** Starting ralph/autopilot, or treating tracker output as acceptance.
- Informational questions (`what is omg-trace?`) do **not** activate this skill.
...
## 3. Runtime owner
- omg-cli tracker. This skill only reads and writes artifact notes.
...
- run_id if known; otherwise `omg state`. Notes under `.omg/artifacts/trace/`.
...
- `omg tracker status|project|reconcile` — passive generation-fenced projection.
