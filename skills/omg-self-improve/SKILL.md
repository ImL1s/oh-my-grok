---
name: omg-self-improve
description: Write improvement proposals for oh-my-grok docs/PRs. Use when the user says self-improve. There is no runtime learning loop.
---

# omg-self-improve — Self-improve proposals (no learning loop)

Write improvement proposals for oh-my-grok docs/PRs. Use when the user says self-improve. There is no runtime learning loop.
...
## 1. Activation
- Phrases: `self improve`, `self-improve`
- Aliases: `self-improve`
- **When:** User wants a retrospective or process improvement write-up.
- **Do not use when:** Claiming the agent modified its own weights/runtime, or auto-merging to main.
- Informational questions (`what is omg-self-improve?`) do **not** activate this skill.
...
## 3. Runtime owner
- This playbook (read-only). Honesty: no OMG learning loop exists.
...
- What failed + proposed doc/test/playbook change.
...
- No CLI twin. Output `.omg/artifacts/self-improve.md` only.
