...
# Install the REVIEWED committed standalone — do NOT regenerate here (installing
# freshly-generated bytes would bypass review). Only VERIFY it is not stale.
if ! python3 "$ROOT/scripts/generate_standalone_hook.py" --check >/dev/null 2>&1; then
  echo "WARN: committed standalone is stale vs omg_cli/deny.py — run" >&2
  echo "      python3 scripts/generate_standalone_hook.py && re-commit before release" >&2
fi
