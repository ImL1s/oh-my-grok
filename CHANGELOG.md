...
- **#147 leftover: TOCTOU promote, task inbox, interactive scale:**
  ...
  snapshots from `start_team`. Canonical scale specs keep subject and
  depends-on. Interactive scale refuses non-grok/fixture routes.
  `team resume` persists and returns relaunch readiness. Mixed interactive/headless active sets
...
- **#73 leftover: hermetic SCIP protobuf Index:** CodeGraph `index`
  ...
  symbols use the SCIP grammar (`omg . . . path/name.`), not
  `path#name`. Status/query load occurrences from that protobuf
  (`not_scip: false` when it decodes). JSON-lite stays `not_scip: true`.
  PATH Homebrew MIP `scip` (scipopt) is classified and refused as
  Sourcegraph SCIP. Not live Antigravity MCP. Never `verified`.
  Refs #73 (does not close).
- **#73 leftover: atomic CodeGraph JSON+SCIP publish:** `codegraph_index`
