...
# omg-autoresearch-goal — Autoresearch against a goal ledger

Research in service of an ultragoal ledger. Use when the user says autoresearch-goal.
...
## 1. Activation
- Phrases: `autoresearch goal`, `autoresearch-goal`
- Aliases: `autoresearch-goal`
- **When:** A durable goal exists (or will) and research must attach to it.
- **Do not use when:** Inventing host `/goal` status from hooks.
- Informational questions (`what is omg-autoresearch-goal?`) do **not** activate this skill.
...
- `omg goal *` for the ledger. Research notes under `.omg/artifacts/autoresearch-goal/`.
