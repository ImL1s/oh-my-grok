---
name: omg-ecomode
description: OMG skill projection (omg_native, none)
omg_projection: true
omg_classification: omg_native
omg_capability_mode: none
omg_source_skill: skills/omg-ecomode/SKILL.md
---
# PROJECTION — not an installed Antigravity plugin

This file is a static parity projection of the Grok plugin skill
`skills/omg-ecomode/SKILL.md`. It is not an installed Antigravity plugin,
not live AG evidence, and does not mean `agy` skill discovery works.

- Catalog: `skills/catalog.json`
- capability_mode: `none` (never `execute`/`all`)
- Playbook without runtime evidence stays `configured`, not `verified`.

# omg-ecomode — Eco/token posture (not a spawn mode)

Prefer cheaper/shorter context. Use when the user says ecomode or eco mode. capability_mode stays none — never execute/all.
...
## 1. Activation
- Phrases: `ecomode`, `eco mode`
- Aliases: `ecomode`
- **When:** User wants a low-token posture for this session.
- **Do not use when:** A sandbox, a continuation owner, or a new CLI mode FSM.
- Informational questions (`what is omg-ecomode?`) do **not** activate this skill.
...
## 3. Runtime owner
- This playbook. capability_mode `none` in the catalog — still never spawn execute/all.
...
- No CLI twin. Write `.omg/artifacts/ecomode.md` with the posture you will follow (shorter diffs, fewer spawns, min-context handoffs).

## 6. capability_mode
- This skill's catalog mode: `none` (never `execute` / `all`).
