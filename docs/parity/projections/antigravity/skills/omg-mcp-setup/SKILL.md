...
omg_source_skill: skills/omg-mcp-setup/SKILL.md
---
# PROJECTION — not an installed Antigravity plugin

This file is a static parity projection of the Grok plugin skill
`skills/omg-mcp-setup/SKILL.md`. It is not an installed Antigravity plugin,
...
# omg-mcp-setup — MCP install helper

Install or print the Grok MCP add command. Use when the user says mcp setup or install mcp.
...
## 1. Activation
- Phrases: `mcp setup`, `mcp-setup`, `install mcp`
- Aliases: `mcp-setup`
- **When:** Need the plugin MCP registration on PATH via grok mcp add.
- **Do not use when:** Inventing lsp.* tools on omg mcp-server, or claiming MCP is live-verified.
- Informational questions (`what is omg-mcp-setup?`) do **not** activate this skill.
...
## 3. Runtime owner
- omg-cli. Config notes under `.omg/artifacts/mcp-setup.md`.
...
- Whether grok is on PATH; print-only vs apply.
