...
# omg-project-session-manager — Project session manager

Allocate or inspect named sessions without inventing UUIDs. Use when the user says project session manager, psm, or session manager.
...
## 1. Activation
- Phrases: `project session manager`, `session manager`
- Aliases: `project-session-manager`
- **When:** Need exact create/resume/continue/fork argv for a named child session.
- **Do not use when:** Reusing named child UUIDs, or treating session allocate as verified.
- Informational questions (`what is omg-project-session-manager?`) do **not** activate this skill.
...
## 3. Runtime owner
- omg-cli session. Notes under `.omg/artifacts/session-manager.md`.
...
- Desired action + existing session id if any.
...
- `omg session allocate|route`. Alias `psm` resolves here.
