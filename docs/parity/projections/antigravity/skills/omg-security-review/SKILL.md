...
# omg-security-review — Security review lane (not verified)

Read-only security review of a change. Use when the user says security review. Writes a verdict proposal only.
...
## 1. Activation
- Phrases: `security review`, `security-review`
- Aliases: `security-review`
- **When:** Need OWASP/secrets/unsafe-pattern review of a diff.
- **Do not use when:** Self-approving, writing verified, or replacing dual-review.
- Informational questions (`what is omg-security-review?`) do **not** activate this skill.
...
## 3. Runtime owner
- `omg-security-reviewer` spawn `read-only`. CLI owns any review stamp.
...
- Diff scope + threat notes. Artifact: `.omg/artifacts/security-review.md`.
...
- `omg review` / `omg dual-review` for gated critic→verifier. This skill adds a security lane proposal.
