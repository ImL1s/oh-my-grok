...
# omg-external-context — External context ingest (memory proposals)

Pull external notes into project memory proposals. Use when the user says external context or bring in context.
...
## 1. Activation
- Phrases: `external context`, `bring in context`
- Aliases: `external-context`
- **When:** Need to capture outside facts without treating them as verified run state.
- **Do not use when:** Silent overwrite of `.omg/state/`, or launching a continuation owner.
- Informational questions (`what is omg-external-context?`) do **not** activate this skill.
...
## 3. Runtime owner
- omg-cli memory. This skill proposes; CLI writes the store.
...
- Source + intended fact keys. Never paste secrets.
...
- `omg memory put|search|show|export|import|rescan` (redacted facts). Proposals may also land under `.omg/artifacts/`.
