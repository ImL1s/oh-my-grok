...
# omg-prometheus-strict — Prometheus-strict plan gate

Stricter ralplan posture (steelman + fail-closed schema). Use when the user says prometheus-strict.
...
## 1. Activation
- Phrases: `prometheus strict`, `prometheus-strict`
- Aliases: `prometheus-strict`
- **When:** Plan consensus must be extra-strict before any implement.
- **Do not use when:** Writing product code, or running as a second continuation owner beside ralplan.
- Informational questions (`what is omg-prometheus-strict?`) do **not** activate this skill.
...
## 3. Runtime owner
- omg-cli ralplan. Conflict: refuse another continuation owner.
...
- Goal + interview notes. Artifacts under `.omg/artifacts/prometheus-strict/`.
...
- `omg ralplan` (same CLI twin; this skill is the stricter playbook).
