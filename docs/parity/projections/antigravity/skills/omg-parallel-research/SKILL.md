...
# omg-parallel-research — Parallel research fan-out

Fan out independent research slices at depth=1. Use when the user says parallel research or sciomc (alias).
...
## 1. Activation
- Phrases: `parallel research`, `sciomc`
- Aliases: none
- **When:** Several independent questions can be explored concurrently.
- **Do not use when:** Implementer fan-out (that's ultrawork), or nested spawn.
- Informational questions (`what is omg-parallel-research?`) do **not** activate this skill.
...
## 3. Runtime owner
- This playbook. Each child `read-only`, depth=1, no child spawn.
...
- List of independent questions. Min-context: one brief per slice + merge.
...
- `omg ask` optional. Alias `sciomc` resolves here. Merge notes under `.omg/artifacts/parallel-research/`.
