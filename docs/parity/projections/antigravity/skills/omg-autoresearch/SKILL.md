...
# omg-autoresearch — Autoresearch loop (artifacts only)

Bounded research iterations into artifacts. Use when the user says autoresearch. No dedicated CLI twin.
...
- **When:** Need repeated read-only research passes without a continuation owner.
- **Do not use when:** A persistence loop that self-sets verified, or replacing ultragoal.
- Informational questions (`what is omg-autoresearch?`) do **not** activate this skill.
...
## 3. Runtime owner
- This playbook. Spawn explore `read-only`, depth=1.
...
- Research question + stop condition (N passes or artifact complete).
...
- No CLI twin. Optional `omg ask` for a human-brokered opinion. Write `.omg/artifacts/autoresearch/`.
