...
# omg-configure-notifications — Notification queue configure

Configure outbound-only notify delivery. Use when the user says configure notifications or setup notifications.
...
## 1. Activation
- Phrases: `configure notifications`, `setup notifications`
- Aliases: `configure-notifications`
- **When:** Need status/send/process of the non-authoritative notify queue.
- **Do not use when:** Treating notify delivery as run acceptance.
- Informational questions (`what is omg-configure-notifications?`) do **not** activate this skill.
...
## 3. Runtime owner
- omg-cli notify. Config/proposal under `.omg/artifacts/notify/`.
...
- owner_id + generation + channel constraints. No secrets in chat logs.
