...
# omg-writer-memory — Writer memory / notes

Capture writer-facing notes into memory or omg note. Use when the user says writer-memory.
...
## 1. Activation
- Phrases: `writer memory`, `writer-memory`
- Aliases: `writer-memory`
- **When:** Need durable writer notes that are not run verified stamps.
- **Do not use when:** Dumping secrets, or treating notes as acceptance.
- Informational questions (`what is omg-writer-memory?`) do **not** activate this skill.
...
- Note body + optional memory keys.
