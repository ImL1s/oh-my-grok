...
- Historical `to_ref` pin-transition edges reconstruct the plan and the canonical generated-doc digest from committed blobs at that ref. They never take `generated_docs_hash` from a candidate receipt, never glob `GROK_BUILD-from-to-*.json`, and never advertise a change-digest filename when a receipt is missing. If the digest cannot be recomputed, the gate fails closed.
- Claim-gate git invocations used for committed-blob identity pass `--no-replace-objects` and a sanitized environment that drops `GIT_DIR`, `GIT_WORK_TREE`, `GIT_INDEX_FILE`, `GIT_OBJECT_DIRECTORY`, `GIT_ALTERNATE_OBJECT_DIRECTORIES`, and related repository/object overrides.
