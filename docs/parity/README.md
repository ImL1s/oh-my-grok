...
| [`projections/antigravity/agents/`](projections/antigravity/agents/) | Static Antigravity `agent.md` projections of plugin agents (#71). Not an installed AG plugin; not live evidence. |
...
OMC has a committed policy/mapping/proof triple at the pinned revision; canonical `source_status.OMC` remains **bootstrapping** (proof present, unpromoted). OMX likewise has a committed policy/mapping/proof triple at pin `435d4a9…` with `source_status.OMX` still **bootstrapping**. OmO has a committed policy/mapping/proof triple at pin `4ca872b…` with `source_status.OmO` still **bootstrapping**. Antigravity has a committed documentation-only policy/mapping/proof triple at pin `bfab12da…` with `source_status.Antigravity` still **bootstrapping** (docs/catalog seed only — not implementation-complete; stronger upstream still needed). See [completeness-schema-v1.md](completeness-schema-v1.md) for artifact consistency vs source reproduction vs promotion.
...
`complete` on `source_status` / `category_status` requires a **reproducible upstream completeness proof** (catalogue seed ≠ completeness). OMC, OMX, OmO, and Antigravity discovery evidence is committed under `completeness/{policies,mappings,proofs}/{OMC,OMX,OmO,Antigravity}.json` but remains unpromoted. Antigravity's triple is documentation/catalog seed only — not an implementation-registry proof. See the inventory snapshot and [`GAPS.md`](GAPS.md) for current status. Canonical statuses stay `bootstrapping` until statuses are explicitly promoted.
