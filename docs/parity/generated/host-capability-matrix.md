...
| `grok.dashboard.auto_recap_no_interleave` | dashboard | `consumed_downstream` | `catalogued` | #68 | Auto recaps do not interleave with a busy/new turn |
...
| `grok.subagent.cancel_no_restart` | subagent | `consumed_downstream` | `catalogued` | #68, #69 | Esc/stop prevents cancelled background work from restarting the model |
| `grok.subagent.parent_continue_reminder` | subagent | `consumed_downstream` | `catalogued` | #68, #69 | After a background subagent starts, the parent is reminded to continue its original work |
...
| `grok.workflow.parallel_child_cap` | workflow | `consumed_downstream` | `catalogued` | #68, #69 | Live workflow parallel() fan-out is capped instead of launching hundreds of children |
