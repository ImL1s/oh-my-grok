...
最后更新：2026-08-01 · Plugin 版本：**0.8.0**
...
2. **Launcher** — 安装成 `$GROK_HOME/hooks/omg_pretool_deny` 可执行 wrapper（grok **1.0.4** 把 JSON `command` 当 argv0 `execvp`，`python3 … || true` 会 ENOENT 后 fail-open）。wrapper shebang 为 LF；内部跑 `<abs-python3> -I -S "<abs-standalone>" || true`。`-I -S` 隔离直译器；`|| true` 仍把 python rc 2 正规成 fail-open。grok 1.0.4 在 session `--cwd` 为 9p/drvfs（WSL `/mnt/d/…`）时同样 spawn ENOENT；live canary 用隔离临时目录，不用产品 checkout。
3. **In-code** — 整段 `try/except`，任何错误预设 allow。
4. **doctor** — realpath 必须在 `$GROK_HOME` 下 + 真的 `open()` + 行为性子行程 smoke（allow `ls`、allow 第一方 `omg team` 裸命令与路径前缀、deny `claude`、deny 未加 scope 的 `spawn_subagent`）+ installed-vs-committed hash（过期则 WARN；`--strict` 失败）。仍把 `omg team` 当成外部 CLI 的旧 hook 会硬失败，修复命令是 `omg install-hook`。不要信任 `os.access`（它查 permission bits，不是 TCC）。
...
OMX/Sol 对齐的 root 入口（不是 mode FSM；永不盖 `verified`）：

- **裸／prompt：**以安全默认值启动互动 Grok（不注入 authority）。
- **`--madmax`：**注入 `--always-approve` + `--permission-mode bypassPermissions`（恰好一次）。在 `--` 前 head 拒绝不兼容的 `--safe`／permission modes（`SAFE-01`）。
- **传输策略：**`OMG_LAUNCH_POLICY`／`--direct`／`--tmux`（最后 CLI flag 赢；值 `auto|direct|tmux|detached-tmux`）。auto + TTY + 有 tmux → detached 自有 session 再 attach；auto 无 tmux 警告一次并回退 direct；显式 `--tmux` 在 headless／print 捷径**之前** fail-closed（`E_LAUNCH_TMUX_UNAVAILABLE`／`E_LAUNCH_TTY_REQUIRED`）。在 `$TMUX` 内 → 直接 in-process。**auto**（非显式 `--tmux`）下 headless（`-p`、`--single`，…）保持 direct 以保留 stdout。
- **`--` 边界：**第一个 `--` 之后的 suffix 不透明，绝不扫描 wrapper flags。
- **不**写 `.omg/state`，**不**碰 `verified`／acceptance／ask deny lists。
- Root `--yolo` 仍**只**是 mode 子命令升格 — 不是 madmax 别名。
- **Env 转送：**使用 tmux 时透过 `tmux new-session -e KEY=value` 转送 allowlisted `GROK_*`／`XAI_*`／少数 shell 变数。多使用者机器上优先用 host 身份／profile 密钥。
...
| `omg team resume` | `resume_for_identity` 以同一把 **scale lock** 包住幂等活体对账与 dead-worker relaunch。若 relaunch WAL 待处理，先做精确 relaunch recovery，再做 raw reconciliation；否则先 reconciliation。并行 scale／resume 不会重复 spawn；非 team run 会 fail-closed。**默认绝不 attach**。`--view` 在释放 lock 后恢复精确 Team window/leader（`--json` 永不执行 view）。reconcile / provider-session / tmux-view 分开回报。 |
