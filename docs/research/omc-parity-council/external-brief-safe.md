# FREE EXPLORATION AUDIT (sanitized tokens only for hook safety)
You have FULL free exploration of the codebase and related installs.
Do NOT start workflow modes. This is audit/research only.
Token map if you see them: ULW_ALIAS=parallel-mode, ULTRA_WORK_SKILL=parallel-engine, RALPH_SKILL=persist-loop, RAL_PLAN_SKILL=plan-consensus, AUTO_PILOT_SKILL=full-pipeline, ULTRA_GOAL_SKILL=durable-goals.
Prior multi-Grok reports are optional; challenge them with evidence.
...
Fill HAVE / PARTIAL / MISSING / NEVER for: parallel fan-out, persist loop, plan consensus, full pipeline, dual review, ask broker, team/tmux, Stop pin, resume/context, doctor, cancel, accept/verified, capability isolation, deep-interview, ultraqa, ULTRA_GOAL_SKILL, HUD, wiki, notifications.
