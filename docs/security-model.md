...
| Nested | Refuses launch / start / run / scale / resume / stop / collect / supervisor inside a spawned-worker context (`OMG_TEAM_WORKER` / related markers) with stable `E_TEAM_NESTED_LAUNCH` **before** side effects. Identity-bound `omg team api` reaches the runtime operation matrix (not the external-CLI soft guard). |
