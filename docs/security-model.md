...
Last updated: 2026-08-05 · Plugin version: **0.9.0**
...
| First-party `omg team …` (bare, path-prefixed, `env`/`command`/… wrappers, `sh|bash|zsh -c/-lc`) | **Allow** (soft-gate) | Not an external advisor CLI. Team admission is owned by the OMG Team runtime (actor, operation, identity). |
| Foreign orchestration `omc team …` (same head-normalized forms) | **Deny** | Foreign orchestrator; kept with the external soft guard. |
| Nested `omg team` launch/supervise while process env has `OMG_TEAM_WORKER` / related markers | **Deny** with `E_TEAM_NESTED_LAUNCH` reason | Defense-in-depth only; runtime also refuses before side effects. Command-text `OMG_TEAM_WORKER=0 …` never authorizes. Identity-bound `omg team api …` is not mislabeled as external CLI. |
...
assignment written inside command text as authorization. It also does **not**
authorize first-party Team nested launch (worker process markers still apply).
...
| Nested | Refuses launch / start / run / scale / resume / stop / collect inside a spawned-worker context (`OMG_TEAM_WORKER` / related markers) with stable `E_TEAM_NESTED_LAUNCH` **before** side effects. `main()` now runs the same parsed-argv preflight **before** project-root discovery (so a missing `--project-root` cannot mask the typed `E_TEAM_NESTED_LAUNCH` / `E_TEAM_WORKER_OPERATION_REFUSED` refuse). Hook remains fail-open soft DiD; `cmd_team` remains runtime DiD. Legal pane **`omg team supervisor --descriptor …`** is **not** blanket-refused: leader-materialized panes intentionally set `OMG_TEAM_WORKER=1` and are admitted only via identity-bound descriptor + leader-root validation (and `team.json` owner_token binding when published); missing/forged/stale authority fails closed with zero provider/tmux/state side effects. Soft-gate DiD may still classify shell-text `omg team supervisor` as nested when a worker process tries to re-invoke it via PreToolUse. Identity-bound `omg team api` reaches the runtime operation matrix (not the external-CLI soft guard). |
...
| `omg team resume` | `resume_for_identity` holds one **scale lock** across idempotent liveness reconciliation and dead-worker relaunch. If a relaunch WAL is pending, exact relaunch recovery runs before raw reconciliation; otherwise reconciliation runs first. Concurrent scale/resume cannot double-spawn; non-team runs fail closed. **Default never attaches** or changes the tmux client. `--view` restores the exact Team window/leader **after** the lock is released (identity-fenced; `--json` never executes view effects). `--provider-session` consumes the host `session_resume` gate (CLI injects `FeatureGateResult` with `required=False` so LEGACY is reachable; BLOCKED fails closed). Reconcile / provider-session / tmux-view outcomes are separate. |
...
| Operator pane control (`panes`/`capture`/`focus`/`key`/`input`/`watch`) | Identity-fenced (#101): every effect re-proves exact session/nonce/pane/pid via the #98 receipt chain before tmux argv (`shell=False`). Key allowlist + literal `send-keys -l` only; bounded/redacted capture; audit stores length/hash not raw text. `--json` never focuses or delivers input. **#147 capability gate (PR1):** CLI-written worker I/O state (`io_mode`, `provider_tty_owner`, `input_ready`, `operator_input_supported`, `interaction_evidence`) is fail-closed and independent of pane visibility / `needs_pty` / `startup_status=running`. Current supervisor-owned panes stamp `headless_stream` + `operator_input_supported=false`; `input`/`key` refuse with `E_OPERATOR_INPUT_UNSUPPORTED` / `E_OPERATOR_KEY_UNSUPPORTED` (or `E_OPERATOR_INPUT_NOT_READY` when a future supported path is not ready) **before** any tmux send. `--operator-override` bypasses only CLI TTY policy — never capability refuse. Success never claims provider consumption (`submitted_to_exact_tty` vs `acknowledged_by_provider`; no `delivered:true` overclaim). **#147 PR2:** `--io-mode interactive` execs grok|fixture on the pane TTY (`provider_tty_owner=provider`); default/`auto` stay headless; explicit interactive never silently downgrades. Leader TUI-ready wait (`TUI_READY:<nonce>`) is the interactive launch gate (not supervisor ACK); only the CLI promotes `input_ready`. Prefer durable `omg team api` mailbox/task ops for automation — pane input is operator recovery/steering, not authoritative task evidence or `verified`. |
| Team view / resume `--view` (`view`) | Identity-fenced (#103): pure view planner + exact topology target (#102) + re-probe before `select-*` / `switch-client` / `attach-session`. Default resume is reconcile-only; `--takeover` required for `attach -d`; `--print` never executes; never holds the lifecycle lock across interactive attach. |
