...
agent frontmatter 若省略、使用 snake_case 别名（`capability_mode` / `permission_mode`），或与目录的 `capabilityMode` / `permissionMode` 不一致则 fail-closed。Agent markdown 以 `O_NOFOLLOW` 打开并从 pinned descriptor 读取。
