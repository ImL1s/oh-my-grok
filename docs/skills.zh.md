...
| 指令 | Contract |
|---|---|
| `omg session allocate\|route\|search\|friction\|replay\|observatory\|retain\|ag-history\|acp-resume` | Host-session argv，加上已脱敏的 search/friction/replay/observatory/retention。`ag-history` 在 POSIX 上仅通过已钉选的 immutable descriptor 打开稳定的 Antigravity SQLite summary schema v1；缺少安全 descriptor-relative SQLite 访问的 host 会返回 unsupported-platform 诊断。最多返回 200 条哈希且不含正文的 descriptor；要求由索引约束的 newest-first 查询计划，拒绝 symlink，并在非空 WAL、active rollback journal、来源变动、未知 schema、损坏格式或无界查询计划时安全跳过，不会变更 host DB sidecar。所有 SHM leaf 仍会以 no-follow 方式验证；仅有 stale SHM 不会被拒绝，因为它不包含已提交的数据库内容。Importer 不会选取 title、preview、transcript、工具输出或 app-data 路径。有限的 workspace URI metadata 仅用于哈希与脱敏，绝不输出完整路径。Replay 永不重放命令。`acp-resume` 复用 host ACP initialize+session/resume，只产出无正文 receipt（hermetic `OMG_ACP_BIN` 不是 live Grok；无 session/close；restore-code 一律拒绝）。Refs #74。 |
