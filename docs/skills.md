...
| Command | Contract |
|---|---|
| `omg session allocate\|route\|search\|friction\|replay\|observatory\|retain\|ag-history\|acp-resume` | Host-session argv plus redacted search/friction/replay/observatory/retention. `ag-history` on POSIX opens only a stably WAL-free Antigravity SQLite summary schema v1 through a pinned immutable descriptor; hosts without secure descriptor-relative SQLite access report an unsupported-platform diagnostic. It returns at most 200 hashed/content-free descriptors, rejects symlinks, and skips active WAL, changing, unknown, or corrupt formats rather than mutating host DB/WAL/SHM state; it never selects titles, previews, transcripts, tool output, or app-data paths. Bounded workspace URI metadata is read only for hashing/redaction, and full paths are never emitted. Replay never re-executes commands. `acp-resume` reuses host ACP initialize+session/resume for a content-free receipt (hermetic `OMG_ACP_BIN` is not live Grok; no session/close; restore-code refused). Live `grok agent stdio` `_x.ai/*` notifications are discarded chrome (bodies never logged). Refs #74. |
