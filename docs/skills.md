...
| `omg edit plan\|apply\|comments\|simplify` | Hash-anchored edit CLI plus comment/simplifier hygiene (#76): `plan` is read-only; `apply` calls `apply_hash_edit` after Team ownership / `OMG_CAPABILITY_MODE=read-only` gates; `comments` is report-only unless `--fix`; `simplify` is disabled unless `--enable` or `.omg/simplify.json`. Never writes `passes`/`verified`. Does not claim `omo.edit.hash_anchored` host parity. See `docs/hash-edit.md`. |
