...
| **Honesty** | Zero-config = grok panes; `--routing` enables multi-CLI (codex/agy/cursor/gemini) with role floors. **Integration** isolation only (ownership + seal + integrate) — **not** an execution sandbox (see `docs/security-model.md` posture table). `collect` / `run` / `scale` / `resume` never set `verified`. Scaling/resume/ralph are **lifecycle extensions** of the same team plane (no new isolation claims). Shorthand uses **split-pane** topology + seeds team API (P0′ surface; catalog v6 has 42 named ops / 42 dispatched on hermetic file stores — not live OMX / live AG parity; see `docs/team-operation-catalog-v6.md`). Live promotion proof: `scripts/live_team_smoke.py --live` → `LIVE_TEAM_SMOKE_OK` (2026-07-30 local; not CI-required). **No bare `/team` slash alias** — 2026-07-25 host probe (`grok inspect` / plugin skill docs): skills are `/name` or `/plugin:name`; no frontmatter to register an unnamespaced `/team` for `omg-team`, and other plugins already expose `team` skills. |
...
`docs/team-operation-catalog-v6.md` for catalog v6,
...
| Command | Contract |
|---|---|
| `omg session allocate\|route\|search\|friction\|replay\|observatory\|retain\|ag-history\|acp-resume` | Host-session argv plus redacted search/friction/replay/observatory/retention. `ag-history` on POSIX opens only a stable Antigravity SQLite summary schema v1 through a pinned immutable descriptor; hosts without secure descriptor-relative SQLite access report an unsupported-platform diagnostic. It returns at most 200 hashed/content-free descriptors, requires an index-bounded newest-first query plan, rejects symlinks, and skips a non-empty WAL or active rollback journal, changing sources, unknown schemas, corrupt formats, or unbounded query plans rather than mutating host DB sidecars. Every SHM leaf is still validated without following links, but a stale SHM alone is not rejected because it contains no committed database content. The importer never selects titles, previews, transcripts, tool output, or app-data paths. Bounded workspace URI metadata is read only for hashing/redaction, and full paths are never emitted. Replay never re-executes commands. `acp-resume` reuses host ACP initialize+session/resume for a content-free receipt (hermetic `OMG_ACP_BIN` is not live Grok; no session/close; restore-code refused). Live `grok agent stdio` `_x.ai/*` notifications are discarded chrome (bodies never logged). Refs #74. |
...
| `omg agents list\|explain` | Dual-host agent/model policy inspect (#131) plus host-neutral UX (#134): `--width` / `COLUMNS` narrow-normal-wide, `NO_COLOR`, CJK aliases. Stock Grok Build uses explicit inherit; Medley caps are unsupported (not installation failed) unless `--host-inspect` / `OMG_MEDLEY_INSPECT` supplies a Medley inspect document. Unset inspect is `inspect_source=absent` with `attempt` null; Medley #18 fallback is not attempted. No paid probe. Medley TUI remains #290. |
...
| `omg visual compare\|capture\|verdict\|ralph\|overlay` | Visual Contract V1 (#75). `compare` wraps `compare()` on `--input` JSON (scored/blocked; callers compare `aggregate` to `threshold`; contract stays pixel-agnostic). `capture` runs `capture.command` else `OMG_VISUAL_CAPTURE`, else **blocked** (not a fake pass; Playwright is not required). `verdict` wraps `compare()`, writes descriptors/findings/score history under `.omg/artifacts/visual/<run_id>/`, and `reviewer_status` requires an independent read-only reviewer (`E_VISUAL_REVIEWER` otherwise). `ralph` is a bounded capture/verdict/repair-prompt loop (no agent spawn). `overlay` decodes PNG (stdlib only) and writes numeric stats + `overlay.png` (`pixel_decode: true`; `--descriptor-only` skips decode; fail-closed on truncated/symlink/non-PNG). Never writes `passes`/`verified`. No live screenshot smoke / no AG vision model in this slice. See [visual-contract-v1.md](./visual-contract-v1.md). |
| `omg tools doctor\|serve\|lsp\|ast\|codegraph\|research` | OMG-owned sidecar (#73): semantic LSP / AST-grep / CodeGraph (toy import/symbol scan plus hermetic SCIP protobuf Index beside JSON-lite; JSON-only stays `not_scip`; Homebrew MIP `scip` is refused) / opt-in research. **Not** Grok-native LSP (`omg lsp` stays host-owned). **Not** live Antigravity evidence. `omg mcp-server` still forbids `lsp.*`. See [tools-sidecar.md](./tools-sidecar.md). |
| `omg job start\|status\|wait\|collect\|cancel\|list\|retry\|auto-retry\|gc\|recover` | Durable background jobs (#68 PR1–PR5 + #69 grok provider): `.omg/jobs/<id>/` store, subprocess runner owning `ProviderAdapter.run`, owner lease/heartbeat observation, explicit `recover` → `lost`, explicit `retry --attempt N` with attempt archive, bounded `auto-retry` scheduler tick (automatic class only; one pass, no daemon), terminal `gc --retention-days`, and `omg ask --background` → job_id. `--provider fake` (hermetic), `--provider antigravity` (fail-closed preflight; stream-json default; evidence artifacts only), `--provider grok` (fail-closed preflight; headless `--prompt-file` only — not an interactive TTY owner). Fake-only flags rejected with Antigravity and grok. Authenticated `agy 1.1.22` durable start/wait/collect smoke passed on 2026-08-31; this does not prove live Team workers or live grok jobs. Remaining live job-backed Team evidence is owned by #69; closed #68 is historical. See `docs/durable-jobs.md`. |
| `omg edit plan\|apply\|verify\|comments\|simplify` | Hash-anchored edit CLI plus comment/simplifier hygiene (#76): `plan` is read-only; `apply` calls `apply_hash_edit` after Team ownership / `OMG_CAPABILITY_MODE=read-only` gates; `verify` re-reads and re-plans without writing (stale/ok); `comments` is report-only unless `--fix`; `simplify` is disabled unless `--enable` or `.omg/simplify.json` (later `--apply-edits` failures roll back prior applies in that invocation). Optional `--provider grok` is proposal-only: same assignment/guard, then a Jobs grok job that must emit hash-edit descriptors JSON (never applies; never `verified`). Never writes `passes`/`verified`. Does not claim `omo.edit.hash_anchored` host parity. See `docs/hash-edit.md`. |
...
`--host-inspect` / `OMG_MEDLEY_INSPECT`; inspect-absent keeps `attempt` null
and does not attempt Medley #18 fallback; live spawn/TUI remain Refs). Grok built-ins (`explore`, `plan`, `general-purpose`) are policy
...
| omg-visual-verdict / visual-ralph | `visual compare\|capture\|verdict\|ralph\|overlay` | **never** |
