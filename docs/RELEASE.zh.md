...
| Version | **0.7.6** |
| Intended tag | `v0.7.6` |
| Public assets | `oh-my-grok-0.7.6.tar.gz`，再来是 `SHA256SUMS` |
...
`omg team` **默认开启**（关闭：`OMG_DISABLE_TMUX_TEAM=1`；旧 `OMG_EXPERIMENTAL_TMUX_TEAM=0` 也会关）。Hermetic 传输证明：`scripts/live_team_smoke.py --fixture-executor` → `FIXTURE_TEAM_SMOKE_OK`。Grok-live 升格证明：`scripts/live_team_smoke.py --live` → `LIVE_TEAM_SMOKE_OK`（quota；2026-07-30 本地已绿；**不要**只靠 fixture 宣称 Grok-live 对等）。不是完整 OMX catalog parity（v1：36 named / 25 implemented；见 `docs/team-operation-catalog-v1.md`）。
...
python3 scripts/release_attest.py \
  --asset .omg/artifacts/dual-parity/RUN_ID/OMG-W6/release-bundle/oh-my-grok-0.7.6.tar.gz \
...
```bash
TAG=v0.7.6
curl -fLO "https://github.com/ImL1s/oh-my-grok/releases/download/${TAG}/oh-my-grok-0.7.6.tar.gz"
...
bash install.sh --offline --archive ./oh-my-grok-0.7.6.tar.gz \
...
GitHub release 是 OMG 宣称的发布通道。xAI marketplace PR 仍属可选，且需要该登录当前 schema 加上精确 tag SHA。PyPI／非 editable wheel 与 npm 式套件登录**不是** OMG 0.7.5 宣称的发布通道。发布说明不要暗示否则。
