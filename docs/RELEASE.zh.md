...
| Version | **0.7.3** |
| Intended tag | `v0.7.3` |
| Public assets | `oh-my-grok-0.7.3.tar.gz`，再来是 `SHA256SUMS` |
...
GitHub release 是 OMG 宣称的发布通道。xAI marketplace PR 仍属可选，且需要该登录当前 schema 加上精确 tag SHA。PyPI／非 editable wheel 与 npm 式套件登录**不是** OMG 0.7.3 宣称的发布通道。发布说明不要暗示否则。
