...
- 增強 native 路由的 **Grok 側** `omg agents list` / `explain` 已出貨（read-only）。
  Medley exact／candidates／receipts 與 `/agents` TUI 仍為**規劃中**
  （[medley#287](https://github.com/ImL1s/medley/issues/287) /
  [medley#290](https://github.com/ImL1s/medley/issues/290)），**尚未出貨**。
...
- `purpose` = `advisory` | `task_execution`（權威任務參與；與 read-only / read-write posture **獨立**。read-only / read-write posture 是 **requested OMG posture**，僅在所選 runtime/provider 提供 **qualified enforcement** 時生效；否則為 **unproven** / **unsupported**，或 route 被 blocked。不要把 role floor 當成 sandbox）
...
| `omg agents list` | **已出貨** — Grok baseline inspect；Medley facts 為 unsupported／unavailable |
| `omg agents explain <agent-or-profile>` | **已出貨** — 同上。Medley `/agents` TUI（#290）仍是 **contract-only**，**今日不可跑** |
...
- OMG：[#134](https://github.com/ImL1s/oh-my-grok/issues/134) 的 CLI／JSON／doctor／Team presentation 投影 **已出貨**；Medley TUI 仍是 planned / **contract-only**，**今日不可跑**
...
CLI narrow-width／no-color **已出貨**；Medley TUI 無障礙仍是 **contract-only**，本頁不宣稱 TUI runtime。
