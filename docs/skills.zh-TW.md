...
| 指令 | Contract |
|---|---|
| `omg session allocate\|route\|search\|friction\|replay\|observatory\|retain\|ag-history\|acp-resume` | Host-session argv，加上已脫敏的 search/friction/replay/observatory/retention。`ag-history` 在 POSIX 上僅透過已釘選的 immutable descriptor 開啟穩定的 Antigravity SQLite summary schema v1；缺少安全 descriptor-relative SQLite 存取的 host 會回報 unsupported-platform 診斷。最多回傳 200 筆雜湊且不含正文的 descriptor；要求由索引約束的 newest-first 查詢計畫，拒絕 symlink，並在非空 WAL、active rollback journal、來源變動、未知 schema、損毀格式或無界查詢計畫時安全略過，不會變更 host DB sidecar。所有 SHM leaf 仍會以 no-follow 方式驗證；僅有 stale SHM 不會被拒絕，因為它不含已提交的資料庫內容。Importer 不會選取 title、preview、transcript、工具輸出或 app-data 路徑。有限的 workspace URI metadata 僅供雜湊與脫敏，絕不輸出完整路徑。Replay 永不重放命令。`acp-resume` 重用 host ACP initialize+session/resume，只產出無正文 receipt（hermetic `OMG_ACP_BIN` 不是 live Grok；無 session/close；restore-code 一律拒絕）。Refs #74。 |
...
讀寫角色有編輯工具，只有 orchestrator 有 `run_command`，且只有讀寫視覺實作者有 `generate_image`。
