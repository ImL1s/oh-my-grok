...
| `omg edit plan\|apply\|comments\|simplify` | Hash-anchored 編輯 CLI 加上 comment/simplifier 衛生（#76）：`plan` 唯讀；`apply` 在 Team 所有權 / `OMG_CAPABILITY_MODE=read-only` 門之後走 `apply_hash_edit`；`comments` 預設只報告，除非 `--fix`；`simplify` 預設關閉，除非 `--enable` 或 `.omg/simplify.json`。不寫 `passes`/`verified`。不宣稱 `omo.edit.hash_anchored` 宿主對等。見 `docs/hash-edit.md`。 |
