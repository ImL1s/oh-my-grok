...
- **`omg team resume --run ID`** — 同一 scale lock 下重讀 `team.json`；若 relaunch WAL 待處理，先做精確 relaunch recovery，再做 pane 存活對帳（冪等 status 寫入）。仍符合 receipt 身分的 remain-on-exit dead pane，process 不在時可清理後提交為 `needs_collect`。**預設絕不 attach / 切換 tmux client**（腳本安全）。加 `--view` 在釋放 lifecycle lock 後恢復精確 Team window/leader（同 session `select-*`、跨 session `switch-client`、TTY 外 `attach-session`；`--takeover` 才加 `-d`；`--json` 永不執行 view）。`omg team view` 只恢復視圖不做 reconcile/relaunch；`--print` 只列印 argv。AVAILABLE 時啟動/複用 jobs 平面內部 ACP stdio sidecar（no-replay receipt；非 session/close）。reconcile / provider-session / tmux-view 結果分開回報。
