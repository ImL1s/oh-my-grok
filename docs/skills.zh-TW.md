...
agent frontmatter 若省略、使用 snake_case 別名（`capability_mode` / `permission_mode`），或與目錄的 `capabilityMode` / `permissionMode` 不一致則 fail-closed。Agent markdown 以 `O_NOFOLLOW` 開啟並從 pinned descriptor 讀取。
